--[[
    VEX HUB — PREMIUM BLACK-GREEN MENU (CoreGUI)
    v2.3 — с отладкой полёта
]]
...
main.Active           = false       -- не глотать тачи, когда окно открыто
...
local function startFly()
    ...
    if not hrp then
        warn("[VexHub Fly] HRP отсутствует")
        return
    end

    if hrp.Anchored then
        hrp.Anchored = false
        print("[VexHub Fly] hrp был Anchored, снимаю")
    end

    -- Переключаем Hum в Physics, чтобы он не мешал BV.
    -- Родной джойстик при этом продолжает работать (GetMoveVector).
    if hum then
        pcall(function()
